...
RecId BPlusTree::bPlusSearch(int relId, char attrName[ATTR_SIZE], Attribute attrVal, int op) {
    ...
    if (ret != SUCCESS) {
    std::cout << "search index not available check bplussearch and related";
    }
    ...
    if (ret != SUCCESS) {
        std::cout <<"cant get the attrCatEntry! check on bplussearch this is there";
    }
    // declare variables block and index which will be used during search
    int block, index;

    if (searchIndex.block == -1 and searchIndex.index==-1/* searchIndex == {-1, -1}*/) {
    ...
    */
    ...
    while(StaticBuffer::getStaticBlockType(block) == IND_INTERNAL/* block is of type IND_INTERNAL */) {  //use StaticBuffer::getStaticBlockType()
        ...
        if (op == NE or op == LT or op == LE/* op is one of NE, LT, LE */) {
            /*
            ...
            */
            ...
            if (found/* such an entry is found*/) {
